...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/zuzuq/permission/main/ipmini > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/zuzuq/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/zuzuq/permission/main/ipmini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/zuzuq/permission/main/ipmini | grep $MYIP | awk '{print $3}')
fi
...
clear                    
		    ...
		    echo -e "${Kcyan}│${NC} ${white} DOMAIN  ${NC}: $domain"
                    ...
                    echo -e "${Kcyan}│${NC}${Kbiru} [•4]${NC}📨 ${white} TROJAN-GO${NC}  " "${Kbiru}[•9]${NC}📨 ${white} CHANGE DOMAIN${NC}"   "${Kcyan}│${NC}"
